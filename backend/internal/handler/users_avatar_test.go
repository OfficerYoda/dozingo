...
func TestUploadAvatar_MissingAvatarField_422(t *testing.T) {
	...
	if fakeUploader.uploadCount() != 0 {
		t.Errorf("expected 0 uploads when avatar field is missing, got %d", fakeUploader.uploadCount())
	}
}
...
func TestUploadAvatar_Success_PNG(t *testing.T) {
	...
	// Uploader observed exactly one call with the expected payload.
	if c := fakeUploader.uploadCount(); c != 1 {
		t.Fatalf("expected 1 upload, got %d", c)
	}
...
func TestUploadAvatar_Success_OverwritesPreviousKey(t *testing.T) {
	...
	if c := fakeUploader.uploadCount(); c != 2 {
		t.Fatalf("expected 2 uploads, got %d", c)
	}
...
func TestUploadAvatar_UploaderFails_500(t *testing.T) {
	...
	// avatar_key. A fresh user starts at the column default ("default.svg",
	// see migration 000013), so we expect that value to remain.
	user := loadUserByID(t, userID)
	if user.AvatarKey != "default.svg" {
		t.Errorf("expected AvatarKey to remain at default 'default.svg' after failed upload, got %q", user.AvatarKey)
	}
}

/// ===== avatar_url surfacing on /users/me =====

func TestMe_AvatarURL_NilWhenNoKey(t *testing.T) {
	setupTest(t)
	...
	var got map[string]any
	decodeJSON(t, w, &got)
	// Fresh user has no custom avatar; the DB column defaults to
	// 'default.svg', so /me reports the default avatar URL rather than a
	// null avatar_url. The URL builder produces a non-empty URL whenever
	// avatar_key is non-empty, and the column is NOT NULL with a default.
	urlStr, ok := got["avatar_url"].(string)
	if !ok {
		t.Fatalf("expected avatar_url string from /me for fresh user, got %T (%v)", got["avatar_url"], got["avatar_url"])
	}
	if !strings.HasSuffix(urlStr, "/default.svg") {
		t.Errorf("expected /me avatar_url to point at default.svg, got %q", urlStr)
	}
}
